...
/**
 * Fits the input events to a duration. Events are kept, as the same instances, for as long as
 * their cumulative delta (including their own) does not exceed duration. The first event that
 * would cross duration, and everything after it, is dropped entirely.
 *
 ...
 * @param duration Number of ticks the returned events should span, in total delta
 ...
 * used as a delay, or to pad out events shorter than duration
 * @returns The fitted events, followed by whatever remainder returns
 */
export default function fit(duration: number, input: EventsOrCallable, remainder: Generator<[remainder: number]>): Event[]
{
